...
const cats = [
  ...
  { name: "Города и  все что с ними связано" },
];
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      `https://637db56316c1b892ebd29ec6.mockapi.io/photos?page=${page}&limit=8&${category}` // параметры для отображения количества элементов в пагинации передаем в статическом виде, так лучше не делать
    )
      .then((res) => res.json())
      .then((json) => {
        setCollections(json);
